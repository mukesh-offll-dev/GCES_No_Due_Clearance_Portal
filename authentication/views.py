...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_number(self, page_count):
        ...
        # Line above footer
        width = self._pagesize[0] if hasattr(self, '_pagesize') else A4[0]
        ...
        # Footer content
        self.drawString(margin, 22, "Generated by GCES No Due Clearance Portal")
        page_text = f"Page {self._pageNumber} of {page_count}"
        self.drawRightString(width - margin, 22, page_text)
        self.restoreState()
...
@institution_login_required
def faculty_report_pdf_view(request):
    """
    Generate and download a comprehensive PDF No Due Clearance Report for Faculty Advisor,
    filtered by selected Branch and Year.
    """
    if request.session.get("role") != "FACULTY":
        return HttpResponse("Unauthorized", status=403)
    ...
    if not branch or not year_str:
        return HttpResponse("Branch and Year are required to generate the Faculty Clearance Report.", status=400)

    try:
        year = int(year_str)
    except (ValueError, TypeError):
        return HttpResponse("Invalid Year provided.", status=400)

    # Fetch all students for this branch & year
    try:
        student_docs = list(students_col.find(
            {"branch": branch, "year": year}
        ).sort([("reg_no", 1), ("roll_no", 1)]))
    except Exception as exc:
        logger.error("Failed to query students for faculty report: %s", exc)
        return HttpResponse(f"Database error: {str(exc)}", status=500)

    if not student_docs:
        return HttpResponse(f"No student records found for Branch: {branch}, Year: {year}.", status=404)

    student_ids = [s["_id"] for s in student_docs if "_id" in s]

    # Fetch all dues for these students in one query
    dues_by_student = {}
    if student_ids:
        try:
            for d in no_due_col.find({"student_id": {"$in": student_ids}}):
            ...
            logger.error("Failed to query dues for faculty report: %s", exc)

    # Calculate semesters present in cohort
    semesters = sorted(list({int(s.get("semester")) for s in student_docs if s.get("semester") is not None and str(s.get("semester")).isdigit()}))
    if semesters:
        semester_display = ", ".join([f"Semester {sem}" for sem in semesters])
    else:
        # Fallback based on year
        semester_display = f"Semester {year * 2 - 1} / {year * 2}"

    # Department name
    dept_obj = get_department_by_code(branch)
    dept_name = dept_obj.get("name") if dept_obj else branch

    # Build student row data
    students_data = []
    completed_count = 0

    for idx, s in enumerate(student_docs, start=1):
        sid = s.get("_id")
        s_type = s.get("student_type", "Hosteller")
        is_day_scholar = (s_type == "Day Scholar")
        student_dues = dues_by_student.get(sid, {})

        lib_status = student_dues.get("LIBRARY", "NOT_SENT")
        hostel_status = student_dues.get("HOSTEL", "NOT_SENT")
        college_status = student_dues.get("COLLEGE", "NOT_SENT")
        dept_status = student_dues.get("DEPARTMENT", "NOT_SENT")

        # Required offices for overall status
        required_offices = ["LIBRARY", "COLLEGE", "DEPARTMENT"] if is_day_scholar else ["LIBRARY", "HOSTEL", "COLLEGE", "DEPARTMENT"]
        all_cleared = all(student_dues.get(o) == "APPROVED" for o in required_offices)
        any_rejected = any(student_dues.get(o) == "REJECTED" for o in required_offices)
        any_pending = any(student_dues.get(o) == "PENDING" for o in required_offices)
        ...
        def _fmt_office(st, is_hostel=False):
            if is_hostel and is_day_scholar:
                return "Day Scholar"
            if st == "APPROVED":
                return "Completed"
            elif st == "PENDING":
        ...
        students_data.append({
            "sno": idx,
            "reg_no": str(s.get("reg_no", "")),
            "roll_no": str(s.get("roll_no", "")),
            "name": str(s.get("name", "")),
            "semester": str(s.get("semester", "")),
            "student_type": s_type,
            "library": _fmt_office(lib_status),
            "hostel": _fmt_office(hostel_status, is_hostel=True),
            "college": _fmt_office(college_status),
            "department": _fmt_office(dept_status),
            "overall": overall_status,
        })

    # Prepare response as PDF
    response = HttpResponse(content_type='application/pdf')
    filename = f"Faculty_Clearance_Report_{branch}_Year{year}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    response['Content-Disposition'] = f'attachment; filename="{filename}"'

    # Setup document in Landscape A4 (841.89 x 595.27 pt)
    doc = SimpleDocTemplate(
        response,
    ...
    story.append(Paragraph("Faculty Advisor – Student No Due Clearance Status Report", subtitle_style))
    story.append(Spacer(1, 8))

    # Red divider line (782 width for landscape with 30 margin)
    divider = Table([[""]], colWidths=[782], rowHeights=[2])
    divider.setStyle(TableStyle([
        ('BACKGROUND', (0,0), (-1,-1), primary_color),
        ('BOTTOMPADDING', (0,0), (-1,-1), 0),
        ('TOPPADDING', (0,0), (-1,-1), 0),
    ...
    # Meta-info block
    now_str = datetime.now().strftime("%d-%m-%Y %I:%M %p")
    total_count = len(student_docs)

    info_data = [
        [
            Paragraph("Department / Branch:", info_label_style),
            Paragraph(f"<b>{branch}</b> – {dept_name}", info_val_style),
            Paragraph("Academic Year:", info_label_style),
            Paragraph(f"<b>Year {year}</b>", info_val_style),
            Paragraph("Current Semester:", info_label_style),
            Paragraph(f"<b>{semester_display}</b>", info_val_style),
        ],
        [
            Paragraph("Generated On:", info_label_style),
            Paragraph(now_str, info_val_style),
            Paragraph("Total Students:", info_label_style),
            Paragraph(f"<b>{total_count}</b>", info_val_style),
            Paragraph("Fully Cleared:", info_label_style),
            Paragraph(f"<font color='#16A34A'><b>{completed_count}</b></font> / {total_count}", info_val_style),
        ]
    ]

    info_table = Table(info_data, colWidths=[110, 190, 95, 120, 110, 157])
    info_table.setStyle(TableStyle([
        ('VALIGN', (0,0), (-1,-1), 'MIDDLE'),
    ...
    td_center = ParagraphStyle('FacTableBodyCenter', parent=td_style, alignment=1)
    td_completed = ParagraphStyle('FacCompleted', parent=td_style, textColor=colors.HexColor("#16A34A"), fontName='Helvetica-Bold', alignment=1)
    td_pending = ParagraphStyle('FacPending', parent=td_style, textColor=colors.HexColor("#D97706"), fontName='Helvetica-Bold', alignment=1)
    td_incomplete = ParagraphStyle('FacIncomplete', parent=td_style, textColor=colors.HexColor("#DC2626"), alignment=1)
    td_dayscholar = ParagraphStyle('FacDayScholar', parent=td_style, textColor=colors.HexColor("#2563EB"), alignment=1)
    ...
    table_data = [[
        ...
        Paragraph("Hostel", th_style),
        Paragraph("College", th_style),
        Paragraph("Department", th_style),
        Paragraph("Overall Status", th_style),
    ]]

    def _badge(val):
        if val == "Completed":
            return Paragraph("Completed", td_completed)
        elif val == "Pending":
            return Paragraph("Pending", td_pending)
        elif val == "Rejected":
            return Paragraph("Rejected", td_incomplete)
        elif val == "Day Scholar":
            return Paragraph("Day Scholar", td_dayscholar)
        return Paragraph("Incomplete", td_incomplete)

    for row in students_data:
        table_data.append([
            Paragraph(str(row["sno"]), td_center),
            Paragraph(row["reg_no"], td_center),
            Paragraph(row["roll_no"], td_center),
            Paragraph(row["name"], td_style),
            Paragraph(str(row["semester"]), td_center),
            Paragraph(row["student_type"], td_center),
            _badge(row["library"]),
            _badge(row["hostel"]),
            _badge(row["college"]),
            _badge(row["department"]),
            _badge(row["overall"]),
        ])

    col_widths = [24, 78, 62, 140, 32, 65, 68, 68, 68, 81, 96]
